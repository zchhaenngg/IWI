...
namespace ImproveX.Tests
{
    public abstract class ImproveXTestBase : AbpIntegratedTestBase<ImproveXTestModule>
    {
        ...
        protected void UsingDbContext(Action<ImproveXDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ImproveXDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ImproveXDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ImproveXDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ImproveXDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ImproveXDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ImproveXDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ImproveXDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ImproveXDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ImproveXDbContext, Task<T>> func)
        {
            T result;
